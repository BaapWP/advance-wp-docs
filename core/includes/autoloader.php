 ...
 * plugin.
 */
spl_autoload_register( 'bwp_advance_docs_autoload' );

/**
 * Dynamically loads the class attempting to be instantiated elsewhere in the
 * plugin by looking at the $class_name parameter being passed as an argument.
 *
 * The argument should be in the form: BWP\Namespace. The
...
function bwp_advance_docs( $class_name ) {
	...
	$file_parts = array_map( 'bwp_advance_docs_get_path_part', $file_parts );

	$class_instance_name = array_pop( $file_parts );

	$file_name = bwp_advance_docs_get_file_name($class_instance_name);
...
function bwp_advance_docs_get_path_part( $file_part ) {
	$path_part = strtolower( $file_part );

	return str_ireplace( '_', '-', $path_part );
}

function bwp_advance_docs_get_file_name($class_instance_name){
